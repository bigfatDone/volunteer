    ...
    <div class="content">
      <header>
        <div class="login" v-if="this.$store.state.userInfo.flag == 1">
          <div class="haslogin" @click="status = !status">
            欢迎您，{{ this.$store.state.userInfo.name }}
      ...
      <nav>
        <ul>
          ...
          <router-link
            ...
            to="/publish"
            class="nav-hover"
            v-show=" this.$store.state.userInfo.grade === 3"
          >发布项目</router-link>
          <router-link
            tag="li"
            to="/audit"
...
export default {
  ...
  methods: {
    search() {
      this.$router.push({name:'project',query:{data:this.searchValue,type:1}})
  ...
  .content {
    ...
    header {
      ...
      .login {
        ...
        .volunteer-register:hover,
        .community-register:hover {
          color: red;
        }
      }
      .center {
